<div id="tab-1" class="tab-pane fade show p-0 active">
    <div class="row g-12">
        <div class="col-lg-12">
            <div class="bg-light rounded" style="background-color: #eaeaea !important">
                <div class="border-bottom p-4 mb-4" style="border-bottom: 3px solid #d8d7d7 !important">
                    <h4 class="text-primary-gradient mb-1">Gabah Kering</h4>
                </div>
                <div class="p-4 pt-0" style="color:black">
                    <table class="table table-striped" id="dataGabah" style="color:black">
                        <thead>
                            <tr style="color:black">
                                <th scope="col" class="text-center">No.</th>
                                <th scope="col" class="text-center">Jenis Gabah</th>
                                <th scope="col" class="text-center">Massa (Kg)</th>
                                <th scope="col" class="text-center">Suhu Awal (C)</th>
                                <th scope="col" class="text-center">Kadar Air Awal(%)</th>
                                <th scope="col" class="text-center">Suhu Akhir (C)</th>
                                <th scope="col" class="text-center">Kadar Air Akhir(%)</th>
                                <th scope="col" class="text-center">Durasi(Mnt)</th>
                                <th scope="col" class="text-center">Status</th>
                            </tr>
                        </thead>
                        <tbody style="color-text: black">

                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>

<div id="tab-2" class="tab-pane fade show p-0">
    <div class="row g-9 justify-content-center">
        <div class="col-lg-9">
            <div class="bg-light rounded" style="background-color: #eaeaea !important">
                <div class="border-bottom p-4 mb-4" style="border-bottom: 3px solid #d8d7d7 !important">
                    <h4 class="text-primary-gradient mb-1">Gabah Basah</h4>
                </div>
                <div class="p-4 pt-0" style="color:black">
                    <table class="table table-striped" id="datatable2" style="color:black">
                        <thead>
                            <tr style="color:black">
                                <th scope="col" class="text-center">No.</th>
                                <th scope="col" class="text-center">Jenis Gabah</th>
                                <th scope="col" class="text-center">Massa (Kg)</th>
                                <th scope="col" class="text-center">Suhu Awal (C)</th>
                                <th scope="col" class="text-center">Kadar Air Awal(%)</th>
                                <th scope="col" class="text-center">Suhu Akhir (C)</th>
                                <th scope="col" class="text-center">Kadar Air Akhir(%)</th>
                                <th scope="col" class="text-center">Durasi(Mnt)</th>
                                <th scope="col" class="text-center">Status</th>
                            </tr>
                        </thead>
                        <tbody style="color-text: black">

                        </tbody>
                    </table>
                    
                </div>
            </div>
        </div>
    </div>
</div>